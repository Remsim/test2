...
public class SeleniumLogger {
    ...
    private String _logDir = Settings.getLogDirectory();
    ...
    /**
     * Captures a screen shot and attempt to save the image twice into 
     * logs/pictures. 
     */
    public static void screenShot(String name){
    	StringBuilder path = new StringBuilder(Settings.getScreenShotDir()).append(name);
    	File picture = ((TakesScreenshot)
    			SeleniumDriver.getDrivr())
    			.getScreenshotAs(OutputType.FILE);
    	try {
			FileUtils.copyFile(picture, new File(path.toString()));
		} catch (IOException e) {
			logError("Picture Not Saved, Trying again...");
			try {
				FileUtils.copyFile(picture, new File(path.toString()));
			} catch (IOException e1) {
				logError(String.format("Screens shot not saved for %s", name));
			}
		}
    }
}
